...
// it is a background worker there is no one to callback or response to
workers.gatherAllChecks = () => {
    // Get all the checks
    _data.list('checks', (err, checks) => {

        // this IF is needed only for my repo
        const findGitKeepIndex = checks.indexOf('.gitkeep')
        if (findGitKeepIndex > -1) {
            checks.splice(findGitKeepIndex, 1);
        }
...
workers.init = () => {
    ...
    // modify the number (33) to change the color
